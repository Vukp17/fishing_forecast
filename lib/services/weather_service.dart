...
class WeatherService {
   static Future<List<HourlyWeather>> fetchHourlyWeather({double? latitude, double? longitude}) async {
    try {
      final apiUrl = 'https://api.open-meteo.com/v1/forecast?latitude=$latitude&longitude=$longitude&current=temperature_2m,wind_speed_10m&hourly=temperature_2m,relative_humidity_2m,wind_speed_10m';
          ...
          if (time.hour == currentHour && time.difference(now).inDays <= 4) {
            hourlyWeatherList.add(
              HourlyWeather(
                ...
                humidity: (hourlyData['relative_humidity_2m'][i] as num).toDouble(),
                windSpeed: (hourlyData['wind_speed_10m'][i] as num).toDouble(),
              ),
            );
          }
        }
